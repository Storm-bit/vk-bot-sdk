package ru.romanboycov.bots.sdk.utils;

import ru.romanboycov.bots.sdk.utils.vkapi.Auth;
...
public class Utils {
    ...
    public static int sizeOfFile(String url, String dim) {
        ...
        try {
            ...
            LOG.error("IOException when calculating size of file {} , error: {}", url, e.toString());
        }
    ...
    public static void get_hash(Auth auth, String method) {
        String html = auth.session.get("https://vk.com/dev/" + method).send().readToText();
        String hash_0 = regex_search("onclick=\"Dev.methodRun\\('(.+?)', this\\);", html, 1);
        if (hash_0.length() == 0) {
            throw new IllegalStateException("Method is not valid");
        }
        _hashes.put(method, hash_0);
    }

    public static String regex_search(String pattern, String string, int group) {
