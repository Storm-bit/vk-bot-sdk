...
import ru.romanboycov.bots.sdk.clients.Group;
...
public class CallbackApiHandler {
     ...
     */
    public CallbackApiHandler(String path, Client user) {
        ...
        Spark.post(path, (request, response) -> {
            ...
            if (type.equals("confirmation")) {
                ...
                return new JSONObject(group.api().callSync("groups.getCallbackConfirmationCode", user, "group_id", group.getId())).getJSONObject("response").getString("code");
     ...
     * Non-default handler
     */
    public CallbackApiHandler(CallbackApiSettings settings, Client user) {
        ...
        if (settings.getHost() == null || settings.getHost().length() < 2) {
            ...
            while (!server_ok) {
                JSONObject response = new JSONObject(group.api().callSync("groups.setCallbackServer", user, "group_id", group.getId(), "server_url", settings.getHost() + settings.getPath())).getJSONObject("response");
     ...
     */
    public void registerCallback(String name, Client user, Callback callback) {

        if (autoSetEvents) {
            group.api().callSync("groups.setCallbackSettings", user,"group_id", group.getId(), name, 1);
        }
